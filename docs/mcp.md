...
| **network_requests**      | Get captured HTTP requests with automatic issue detection — flags 4xx/5xx failures, transport failures (CORS, DNS, connection) with the browser's reason, duplicate requests, and mixed content. Each entry carries the body it sent, with secret-looking keys redacted; response bodies are not captured. |
